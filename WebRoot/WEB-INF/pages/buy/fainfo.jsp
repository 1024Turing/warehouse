...
<body class="easyui-layout" style="visibility:hidden;">
	    ...
	    <div title="采购单" data-options="region:'north'" style="height:220px ;background-color:;">
	    	<div style="margin-left:20px;margin-top:6px">
	    		<span>操作人 :</span><input type="text" name="buy_name" value="${buy_name }" readonly="readonly">
	    		<span>说     明 :</span><textarea style="resize: none;width:250px;height:38px" name="bo_details"  readonly="readonly">${bo_details }</textarea><br>	    			
	    	</div>
	    	<table border="" id="">
				<tr>
					<th >分类</th>
		...
		<div title="一级审批单" s data-options="region:'center'">
		<c:if test="${fir_result ne '未过' }">
		<form id="faForm" action="${pageContext.request.contextPath}/firstApeAction_save.action" method="post" >
			<input type="hidden" value="${model.fa_id }"  name ="fa_id"/>
			<input type="hidden" value=""  name ="fir_result" id="fir_result"/>
			
		 	<div style="margin-left:20px;margin-top:6px;">
		 		<c:choose>
		 			<c:when test="${fir_result eq '未审批'}">
		 				<span>一级审批人 :</span><input type="text" name="fa_name" value="${user.staff.stf_name }">
		 			</c:when>
		 			<c:otherwise>
		 				<span>一级审批人 :</span><input type="text" name="fa_name" value="${model.fa_name }">
		 			</c:otherwise>
		 		</c:choose>
	    		<span style="margin-left:80px;">审批说 明 :</span><textarea style="resize: none;width:250px;height:38px" name="fa_details"  >${model.fa_details }</textarea><br>	    		
				<br>
	    	</div>
			<table border="" id="">
				<tr>
		...
		<div title=""  data-options="region:'south'"  style="height:40px ;background-color:;" >
			<div style="margin-left:200px;margin-top:5px">
			<c:if test="${fir_result eq '未审批'}">
